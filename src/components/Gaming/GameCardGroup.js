...
const GameCardGroup = ({ title, platform, status, recommendation, description, hoursPlayed, children }) => {
  ...
  const displayDescription = description || recommendation;
  ...
  const games = React.Children.toArray(children)
    ...
    .map((child) => ({
      ...
      recommendation: child.props.recommendation || child.props.description,
