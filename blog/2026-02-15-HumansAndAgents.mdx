...
![Humans and Agents](https://dsanchezcrwebsite.blob.core.windows.net/images/blog/2026-02-15-HumansAndAgents/HumansAndAgents.jpg)
...
- 💬 **Refine intent through prompts and inline guidance**: "Make this function handle null inputs gracefully" is more than a comment; it's a design decision expressed conversationally
- 🔄 **Iterate on design decisions interactively**: exploring multiple implementation approaches before committing to one
- 🗺️ **Explore implementation options**: asking "What are three different ways to implement this caching strategy?" and evaluating tradeoffs in real time
- 🏗️ **Generate scaffolding, tests, and documentation on demand**: reducing time-to-first-iteration from hours to minutes
...
```
Define Intent → Generate → Evaluate → Refine → Accept or Reject → Repeat
```
...
- ✅ **Single responsibility per PR**: one logical change, one review context
- ✅ **Clear linkage to issues or specifications**: every PR should answer "why does this exist?"
- ✅ **Predictable diffs**: reviewers should be able to understand the change in minutes, not hours
- ✅ **Incremental delivery**: ship a series of small, safe changes rather than one monolithic PR
...
- 🔗 **Linked requirements** or work items connecting to the original intent
- 🚦 **Environment or policy gates** that must pass before merge
- 📊 **Confidence signals** from agents (when available) indicating certainty levels
- 🔍 **Review checklists** customized for the type of change
...
- 🆔 **Who authorized this change?** The human who triggered or approved the agent's work
- 🔑 **What permissions did the agent have?** Scoped access following least-privilege principles
- 📋 **Under what policy?** Which rules and constraints governed the agent's behavior
- 🔒 **With what level of privilege?** Read vs. write vs. admin access to different resources
...
4. If tests pass and change is low-risk → auto-merge enabled
5. If tests fail or change is major → human review required
...
1. ✅ **Keep PRs mandatory** for all non-trivial changes, no exceptions for agent-generated code
2. ✅ **Enforce automated checks** consistently, tests, security scans, linting, and compliance
3. ✅ **Use clear labeling** for agent-generated contributions, transparency builds trust
4. ✅ **Maintain strict branch protection** for critical branches, `main` and `production` should be immutable without review

### Tooling

5. ✅ **Configure [CODEOWNERS](https://docs.github.com/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners)** to ensure domain experts review changes in their areas
6. ✅ **Set up [GitHub Environments](https://docs.github.com/actions/deployment/targeting-different-environments/using-environments-for-deployment)** with appropriate approval gates for each deployment target
7. ✅ **Enable [GitHub Advanced Security](https://github.com/enterprise/advanced-security)** for comprehensive scanning on every PR
8. ✅ **Use [GitHub Actions](https://docs.github.com/actions)** to automate the full validation lifecycle

### Culture

9. ✅ **Ensure identities used by agents follow [least-privilege](https://learn.microsoft.com/entra/identity-platform/secure-least-privileged-access) principles**
10. ✅ **Invest in clear documentation** and repository structure to reduce ambiguity for both humans and agents
11. ✅ **Review agent PRs with the same rigor** as human PRs, the standard is the standard
12. ✅ **Normalize giving feedback to AI outputs**, treating suggestions as drafts, not truths
...
- 🔍 **Reviewing code written by non-humans** with the same critical eye applied to human-written code
- 💬 **Giving feedback to prompts and configurations** improving how agents are directed, not just what they produce
- 📝 **Treating agent outputs as drafts, not truths** AI is confident even when wrong
- 🤝 **Sharing patterns and prompt strategies** like sharing code snippets, but for human-agent collaboration
- 📊 **Measuring collaboration effectiveness** tracking agent contribution quality, review times, and incident rates
